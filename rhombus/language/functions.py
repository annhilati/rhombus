...
# For clarification: The functions in this module do not have to exactly mirror the density function types available in vanilla.
...
@BuiltinWizard
def abs(argument: DensityDescriptor) -> Density[types.abs]:
...
@BuiltinWizard
def add(argument1: DensityDescriptor, argument2: DensityDescriptor) -> Density[types.add]:
...
@BuiltinWizard
def blend_density(argument: DensityDescriptor) -> Density[types.blend_density]:
    """Used for smooth transition to chunks generated in old versions.
...
@BuiltinWizard
def cache_2d(argument: DensityDescriptor) -> Density[types.cache_2d]:
...
@BuiltinWizard
def cache_all_in_cell(argument: DensityDescriptor) -> Density[types.cache_all_in_cell]:
...
@BuiltinWizard
def cache_once(argument: DensityDescriptor) -> Density[types.cache_once]:
...
@BuiltinWizard
def clamp(input: DensityDescriptor, min: float, max: float) -> Density[types.clamp]:
...
@BuiltinWizard
def cube(argument: DensityDescriptor) -> Density[types.cube]:
...
@BuiltinWizard
def find_top_surface(density: DensityDescriptor, upper_bound: DensityDescriptor, lower_bound: int, cell_height: int) -> Density[types.find_top_surface]:
...
@BuiltinWizard
def flat_cache(argument: DensityDescriptor) -> Density[types.Reference]:
...
@BuiltinWizard
def half_negative(argument: DensityDescriptor) -> Density[types.half_negative]:
...
@BuiltinWizard
def interpolated(argument: DensityDescriptor) -> Density[types.interpolated]:
...
@BuiltinWizard
def max(argument1: DensityDescriptor, argument2: DensityDescriptor) -> Density[types.max]:
...
@BuiltinWizard
def min(argument1: DensityDescriptor, argument2: DensityDescriptor) -> Density[types.min]:
...
@BuiltinWizard
def mul(argument1: DensityDescriptor, argument2: DensityDescriptor) -> Density[types.mul]:
...
@BuiltinWizard
def quarter_negative(argument: DensityDescriptor) -> Density[types.quarter_negative]:
...
@BuiltinWizard
def range_choice(input: DensityDescriptor, min_inclusive: float, max_exclusive: float, when_in_range: DensityDescriptor, when_out_of_range: DensityDescriptor) -> Density[types.range_choice]:
...
@BuiltinWizard
def shifted_noise(noise: Noise, xz_scale: float, y_scale: float, shift_x: DensityDescriptor, shift_y: DensityDescriptor, shift_z: DensityDescriptor) -> Density[types.shifted_noise]:
...
@BuiltinWizard
def slide(argument: DensityDescriptor) -> Density[types.slide]:
    ...
    """
    return Density(types.slide(argument))

@BuiltinWizard
def spline(coordinate: DensityDescriptor, points: list[tuple[float, DensityDescriptor, float]]) -> Density[types.spline]:
...
@BuiltinWizard
def square(argument: DensityDescriptor) -> Density[types.square]:
...
@BuiltinWizard
def squeeze(argument: DensityDescriptor) -> Density[types.squeeze]:
...
def terrain_shaper_spline(
    ...
    """
    return Density(types.terrain_shaper_spline(spline, min_value, max_value, continentalness, erosion, weirdness))

@BuiltinWizard
def weird_scaled_sampler(rarity_value_mapper: Literal["type_1", "type_2"], input: DensityDescriptor, noise: Noise) -> Density[types.weird_scaled_sampler]:
    ...
    """
    return Density(types.weird_scaled_sampler(noise, rarity_value_mapper, input))
