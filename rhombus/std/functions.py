...
def y_clamped_gradient(
    from_y: int, to_y: int, from_value: float, to_value: float
) -> Density[types.y_clamped_gradient]:
...
def find_top_surface(
...
) -> Density[types.find_top_surface]:
    ...
    """
    return Density(
        types.find_top_surface(density.AST, upper_bound.AST, lower_bound, cell_height)
    )
