...
def add(argument1: AnyDensity, argument2: AnyDensity) -> Density[vdft.add]:
    ...
    """
    return Density(vdft.add(argument1, argument2))

def beardifier() -> Density[vdft.beardifier]:
    ...
    """
    return Density(vdft.beardifier())

def blend_alpha() -> Density[vdft.blend_alpha]:
...
def blend_density(argument: AnyDensity) -> Density[vdft.blend_density]:
    ...
    """
    return Density(vdft.blend_density(argument))

def blend_offset() -> Density[vdft.blend_offset]:
    """Used for smooth transition to chunks generated in old versions.
...
def cache_2d(argument: AnyDensity, *, partition: bool = True) -> Density[vdft.cache_2d]:
    ...
    """
    if partition:
        return Density.partitioned(vdft.cache_2d(argument))
    return Density(vdft.cache_2d(argument))

@builtinmacro
def cache_all_in_cell(argument: AnyDensity, partition: bool = True) -> Density[vdft.cache_all_in_cell]:
    ...
    """
    if partition:
        return Density.partitioned(vdft.cache_all_in_cell(argument))
...
def cache_once(argument: AnyDensity, *, partition: bool = True) -> Density[vdft.cache_once]:
    ...
    """
    if partition:
        return Density.partitioned(vdft.cache_once(argument))
...
def clamp(input: AnyDensity, min: float, max: float) -> Density[vdft.clamp]:
    ...
    **NOTE** [MC-252814](https://bugs.mojang.com/browse/MC/issues/MC-252814): *Clamp density function takes a direct input and doesn't allow a reference*
    ...
    """
    return Density(vdft.clamp(input, min, max))

def constant(argument: float) -> Density[vdft.constant]:
...
def cube(argument: AnyDensity) -> Density[vdft.cube]:
    ...
    """
    return Density(vdft.cube(argument))

def end_islands() -> Density[vdft.end_islands]:
...
def flat_cache(argument: AnyDensity, *, partition: bool = True) -> Density[vdft.Reference]:
    ...
    """
    if partition:
        return Density.partitioned(vdft.flat_cache(argument))
...
def mul(argument1: AnyDensity, argument2: AnyDensity) -> Density[vdft.mul]:
    ...
    **NOTE** that `Infinity * 0` is `NaN`.
    ...
    """
    return Density(vdft.mul(argument1, argument2))

def noise(noise: Noise, xz_scale: float, y_scale: float) -> Density[vdft.noise]:
    """Samples a noise.
    ...
    """
    return Density(vdft.noise(noise, xz_scale, y_scale))

def old_blended_noise(xz_scale: float, y_scale: float, xz_factor: float, y_factor: float, smear_scale_multiplier: float) -> Density[vdft.old_blended_noise]:
...
def range_choice(input: AnyDensity, min_inclusive: float, max_exclusive: float, when_in_range: AnyDensity, when_out_of_range: AnyDensity) -> Density[vdft.range_choice]:
    ...
    """
    return Density(vdft.range_choice(input, min_inclusive, max_exclusive, when_in_range, when_out_of_range))

def shift(argument: Noise) -> Density[vdft.shift]:
    ...
    """
    return Density(vdft.shift(argument))

def shift_a(argument: Noise) -> Density[vdft.shift_a]:
    ...
    """
    return Density(vdft.shift_a(argument))

def shift_b(argument: Noise) -> Density[vdft.shift_b]:
...
def squeeze(argument: AnyDensity) -> Density[vdft.squeeze]:
    ...
    """
    return Density(vdft.squeeze(argument))
...
def weird_scaled_sampler(rarity_value_mapper: Literal["type_1", "type_2"], input: AnyDensity, noise: Noise) -> Density[vdft.weird_scaled_sampler]:
    ...
    """
    return Density(vdft.weird_scaled_sampler(noise, rarity_value_mapper, input))

def y_clamped_gradient(from_y: int, to_y: int, from_value: float, to_value: float) -> Density[vdft.y_clamped_gradient]:
    ...
    """
    return Density(vdft.y_clamped_gradient(from_y, to_y, from_value, to_value))
