...
class FastNoiseConfig(DatapackResource):
    ...
    **NOTE:** Because Lithostitched noises have a lot of interdependent fields, use
    ...
    type: Literal[
        "lithostitched:cellular", "lithostitched:perlin", "lithostitched:simplex"
    ]
    frequency: float
    salt: int = None
    ...
    distance_function: Literal[
        "euclidean", "euclidean_squared", "manhattan", "hybrid"
    ] = None
    return_type: Literal[
        "cell_value",
        "distance",
    ...
    def serialize_toplevel(self) -> JSONDict:
        return {
            ...
            **(
                ...
                if self.type == "lithostitched:cellular"
                else {}
            ),
            **(
                {
                    ...
                    **(
                        ...
                        if self.lacunarity is not None
                        else {}
                    ),
                    **({"gain": self.gain} if self.gain is not None else {}),
                }
                if self.type == "lithostitched:simplex"
                else {}
            ),
        }
