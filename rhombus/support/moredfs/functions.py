__all__ = [
    "when_finite",
    "acos", "asin", "atan", "cbrt", "ceil", "clamp", "cos", "cosh", "derivative",
    "distance", "div", "dot_product", "floor", "floor_div", "floor_mod",
    "gapped_grid_square_spiral", "gradient_magnitude", "ieee_rem", "ln", "log",
    "log2", "log2_floor", "mod", "negate", "or_else", "polar_coords", "power",
    "profiler", "radius", "radius_3d", "reciprocal", "remainder", "resolver",
    "round", "shift", "sigmoid", "signum", "sin", "single_channel_image_tessellation",
    "sinh", "sqrt", "subtract", "tan", "tanh", "value_noise", "vector_angle",
    "x", "x_clamped_gradient", "y", "z", "z_clamped_gradient"
]
...
@macro
def derivative(argument: AnyDensity, component_x: DerivativeComponent | None = None, component_y: DerivativeComponent | None = None, component_z: DerivativeComponent | None = None):
    "Returns the directional derivative."
    return Density(types.derivative(argument.AST, component_x, component_y, component_z))

@macro
def gradient_magnitude(argument: AnyDensity, step_x: int | None = None, step_y: int | None = None, step_z: int | None = None):
    "Returns the magnitude of the gradient vector."
    return Density(types.gradient_magnitude(argument.AST, step_x, step_y, step_z))

@macro
def dot_product(argument1: AnyDensity, argument2: AnyDensity, step_x: int | None = None, step_y: int | None = None, step_z: int | None = None):
    "Returns the dot product of two functions."
    return Density(types.dot_product(argument1.AST, argument2.AST, step_x, step_y, step_z))


#======// Spatial Transformation & Noise //====================================================//

@macro
def shift(argument: AnyDensity, shift_x: AnyDensity, shift_y: AnyDensity, shift_z: AnyDensity):
    "Evaluates `argument` at a shifted position."
    return Density(types.shift(argument.AST, shift_x.AST, shift_y.AST, shift_z.AST))

@macro
def value_noise(sampler: RandomSampler, size_x: int, size_y: int, size_z: int, interpolation: Literal["none", "lerp", "smoothstep"], salt: int | None = None, extra_octaves: ExtraOctaves | None = None):
    "Generates value noise with optional octaves, lacunarity, and persistence."
    return Density(types.value_noise(sampler, size_x, size_y, size_z, interpolation, salt, extra_octaves))
