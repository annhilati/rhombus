...
def terrain_shaper_spline(
    ...
    """
    return Density(types.terrain_shaper_spline(spline, min_value, max_value, continentalness.AST, erosion.AST, weirdness.AST))

@macro
def weird_scaled_sampler(input: AnyDensity, noise: Noise, rarity_value_mapper: Literal["type_1", "type_2"]) -> Density[types.weird_scaled_sampler]:
