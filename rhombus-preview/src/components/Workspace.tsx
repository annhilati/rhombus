...
const Workspace = forwardRef<WorkspaceRef, WorkspaceProps>(({ files, selectedFile, onSelectFile }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    openFile: (file: RhombusContextFile, newTab: boolean) => {
      ...
      const name = file.id.split('/').pop()?.split(':').pop() || 'Unknown';
  ...
  useEffect(() => {
    if (selectedFile && !hasLoadedInitial.current) {
      ...
      const name = selectedFile.id.split('/').pop()?.split(':').pop() || 'Unknown';
  ...
  const onRenderTab = (node: TabNode, renderValues: any) => {
    // We moved the view mode selector to a floating button in TabContentWrapper!
  };
  ...
  const onExternalDrag = (e: React.DragEvent<HTMLElement>) => {
    ...
    if (file) {
      ...
      const existingNode = model.getNodeById(key);
      if (existingNode) {
        return undefined; // Already open, prevent duplicate tab
      }
