    ...
    if (typeof obj.value === 'function' && typeof obj.key === 'function') {
        try {
            ...
            if (e instanceof Error && e.message.startsWith('Missing key in')) {
                // Reformats "Missing key in lithostitched:fast_noise_config: minecraft:hmm" 
                // to "Unknown fast_noise_config reference: minecraft:hmm"
                const match = e.message.match(/^Missing key in ([a-z0-9_.-]+:[a-z0-9_./-]+):\s*(.+)$/i);
                if (match) {
                    const registry = match[1];
                    const refId = match[2];
                    const registryPath = registry.includes(':') ? registry.split(':')[1] : registry;
                    errors.push(`Unknown ${registryPath} reference: ${refId}`);
                } else {
