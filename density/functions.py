...
from density.core.expr import DensityExpression
...
def abs(argument: DensityExpression | float | str):
    ...
    return DensityExpression(dft.abs, argument=argument)

def add(argument1: DensityExpression | float | str, argument2: DensityExpression | float | str):
...
def beardifier():
    ...
    """
    return DensityExpression(dft.beardifier)

def blend_alpha() -> DensityExpression[dft.blend_alpha]:
    ...
    """
    return DensityExpression(dft.blend_alpha)

def blend_density(argument: DensityExpression | float) -> DensityExpression[dft.blend_density]:
    """Used in vanilla for smooth transition to chunks generated in old versions.
    ...
    """
    argument = replace_references(argument)[0]
    return DensityExpression(dft.blend_density, argument=argument)

def blend_offset() -> DensityExpression[dft.blend_offset]:
    """Used in vanilla for smooth transition to chunks generated in old versions.
    ...
    [Minecraft Wiki Reference](https://minecraft.wiki/w/Density_function#blend_alpha)
    """
    return DensityExpression(dft.blend_offset)

def cache_2d(argument: DensityExpression | float):
    ...
    """
    argument = replace_references(argument)[0]
    return DensityExpression(dft.cache_2d, argument=argument)

def cache_all_in_cell(argument: DensityExpression | float):
    """🚨 Should not be used in datapacks.
    ...
    """
    argument = replace_references(argument)[0]
    return DensityExpression(dft.cache_all_in_cell, argument=argument)

def cache_once(argument: DensityExpression | float):
    ...
    """
    argument = replace_references(argument)[0]
    return DensityExpression(dft.cache_once, argument=argument)

def clamp(input: DensityExpression | float, min: float, max: float):
    ...
    return DensityExpression(dft.clamp, input=input, max=max, min=min)
...
def constant(argument: float):
    ...
    """
    return DensityExpression(dft.constant, argument=argument)

def cube(argument: DensityExpression | float):
...
def end_islands():
    ...
    """
    return DensityExpression(dft.end_islands)

def find_top_surface(density: DensityExpression | float, upper_bound: DensityExpression | float, lower_bound: int, cell_height: int):
    ...
    return DensityExpression(dft.find_top_surface, density=density, upper_bound=upper_bound, lower_bound=lower_bound, cell_height=cell_height)

def flat_cache():
    ...

def half_negative(argument: DensityExpression | float):
    ...
    return DensityExpression(dft.half_negative, argument=argument)

def interpolated(argument: DensityExpression | float):
    ...
    """
    argument = replace_references(argument)[0]
    return DensityExpression(dft.interpolated, argument=argument)

def invert(argument: DensityExpression | float):
    """Calculates `1/x`.
    ...
    if isinstance(argument, (int, float)):
        return 1 / argument
    return DensityExpression(dft.invert, argument=argument)

def max(argument1: DensityExpression | float, argument2: DensityExpression | float):
    ...
    return DensityExpression(dft.max, argument1=argument1, argument2=argument2)

def min(argument1: DensityExpression | float, argument2: DensityExpression | float):
    ...
    return DensityExpression(dft.min, argument1=argument1, argument2=argument2)

def mul(argument1: DensityExpression | float, argument2: DensityExpression | float):
...
def noise(noise: Noise, xz_scale: float, y_scale: float) -> DensityExpression[dft.noise]:
    """Samples a noise.
    ...
    """
    return DensityExpression(dft.noise, noise=noise, xz_scale=xz_scale, y_scale=y_scale)
...
def old_blended_noise(xz_scale: float, y_scale: float, xz_factor: float, y_factor: float, smear_scale_multiplier: float):
    ...
    """
    return DensityExpression(dft.old_blended_noise, xz_scale=xz_scale, y_scale=y_scale, xz_factor=xz_factor, y_factor=y_factor, smear_scale_multiplier=smear_scale_multiplier)

def quarter_negative(argument: DensityExpression | float):
    ...
    return DensityExpression(dft.quarter_negative, argument=argument)

def range_choice(input: DensityExpression | float, max_exclusive: float, min_inclusive: float, when_in_range: DensityExpression | float, when_out_of_range: DensityExpression | float):
    ...
    return DensityExpression(dft.range_choice, input=input,max_exclusive=max_exclusive,min_inclusive=min_inclusive,when_in_range=when_in_range,when_out_of_range=when_out_of_range)
...
def shift(argument: Noise):
    ...
    """
    return DensityExpression(dft.shift, argument=argument)
...
def shift_a(argument: Noise):
    ...
    """
    return DensityExpression(dft.shift_a, argument=argument)
...
def shift_b(argument: Noise):
    ...
    """
    return DensityExpression(dft.shift_b, argument=argument)

def shifted_noise(noise: Noise, xz_scale: float, y_scale: float, shift_x: DensityExpression | float, shift_y: DensityExpression | float, shift_z: DensityExpression | float):
    ...
    return DensityExpression(dft.shifted_noise, noise=noise, xz_scale=xz_scale, y_scale=y_scale, shift_x=shift_x, shift_y=shift_y, shift_z=shift_z)

def slide(argument: DensityExpression | float):
    """
...
def spline(coordinate: DensityExpression | float, points: list[tuple[float, float | DensityExpression, float]]):
    ...
    return DensityExpression(dft.spline, spline={"coordinate": coordinate, "points": points})

def square(argument: float | DensityExpression):
...
def squeeze(argument: DensityExpression | float):
    ...
    if isinstance(argument, (int, float)):
        ...
        return x/2 - ((x**3) / 24)
    return DensityExpression(dft.squeeze, argument=argument)

def terrain_shaper_spline(spline: Literal["offset", "factor", "jaggedness"], min_value: float, max_value: float, continentalness: DensityExpression | float, erosion: DensityExpression | float, weirdness: DensityExpression | float):
...
def weird_scaled_sampler(noise: Noise, rarity_value_mapper: Literal["type_1", "type_2"], input: DensityExpression | float):
    ...
    """
    input = replace_references(input)[0]
    return DensityExpression(dft.weird_scaled_sampler, noise=noise, rarity_value_mapper=rarity_value_mapper, input=input)
...
def y_clamped_gradient(from_y: int, to_y: int, from_value: float, to_value: float):
    ...
    """
    return DensityExpression(dft.y_clamped_gradient, from_y=from_y, to_y=to_y, from_value=from_value, to_value=to_value)
