def f(pos1, /, both, *, kwo1, **kwargs):
    ...

# ╭──────────────────────────────────────────────────────────────────────────────────────────╮
# │                                       Terminology                                        │ 
# ╰──────────────────────────────────────────────────────────────────────────────────────────╯
# - say "evaluates" instead of "caluclates" or "returns"

# ╭──────────────────────────────────────────────────────────────────────────────────────────╮
# │                                   Macros & Builtins                                      │ 
# ╰──────────────────────────────────────────────────────────────────────────────────────────╯

func = ...
"""First, there should be a brief scientific description of the process that takes place here.

**NOTE** This should state pretty important things regarding the use of this function, like restrictions on the domain.

Parameters:
    param (type): A description that is a nominal phrase and ands with a period (must meet this condition).


---
💾 This function will ensure proper caching by claiming a separate file.<br>
⚙️ Kind of implementation as nominal phrase, likely with hyperlinks

---
![](data:image/svg+xml;base64,)
`1.18.2` and beyond ┃ [Minecraft Wiki Reference](https://minecraft.wiki/w/Density_function#flat_cache) • [Another Link](#)
"""


# ╭──────────────────────────────────────────────────────────────────────────────────────────╮
# │                                        Functions                                         │ 
# ╰──────────────────────────────────────────────────────────────────────────────────────────╯

d = ...
"""First, there should be a brief scientific description of the process that takes place here.

More detailed descriptions can find place afterwards.

Parameters:
    param (type): A description that is a nominal phrase and ands with a period (must meet this condition).

Returns:
    type: True if at least one bit is set, False otherwise.

Raises:
    Error: if and then
"""

# ╭──────────────────────────────────────────────────────────────────────────────────────────╮
# │                                          Images                                          │ 
# ╰──────────────────────────────────────────────────────────────────────────────────────────╯

# Green Tick:   data:image/svg+xml;base64,
# Blue Tick:    data:image/svg+xml;base64,
# Orange Cross: data:image/svg+xml;base64,

# ╭──────────────────────────────────────────────────────────────────────────────────────────╮
# │                                          Tricks                                          │ 
# ╰──────────────────────────────────────────────────────────────────────────────────────────╯

# ╭──────────────────────────────────────────────────────────────────────────────────────────╮
# │                                         Examples                                         │ 
# ╰──────────────────────────────────────────────────────────────────────────────────────────╯
e1 = ...
"""
*   **Compactness:** Uses 64 times less memory than `List[Bool]`.
*   **Speed:** Offers O(1) time complexity for `set`, `clear`, `test`,
    and `toggle` operations (single word load/store).

Ideal for applications like data-flow analysis, graph algorithms, or
any task requiring dense sets of small integers where memory and
lookup speed are critical.

Example:

```
from std.collections import Counter

var counter = Counter[String].fromkeys(["a", "b", "c"], 1)
print(counter["a"]) # output: 1
```
"""